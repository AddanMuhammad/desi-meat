"use client";

import { useEffect, useRef, useState } from "react";
import { PrimaryButton } from "./ui/primary-button";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";

gsap.registerPlugin(ScrollTrigger);

const SubFooter = () => {
  const sectionRef = useRef<HTMLElement>(null);
  const [vintageEffect, setVintageEffect] = useState<string>("sepia"); // Default effect
  const [isMobile, setIsMobile] = useState(false);

  // Check for mobile on mount and resize
  useEffect(() => {
    const checkMobile = () => {
      setIsMobile(window.innerWidth < 1024); // lg breakpoint
    };

    checkMobile();
    window.addEventListener("resize", checkMobile);

    return () => window.removeEventListener("resize", checkMobile);
  }, []);

  useEffect(() => {
    const ctx = gsap.context(() => {
      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: sectionRef.current,
          start: "top 80%", // Adjusted for better timing
          end: "bottom 20%",
          toggleActions: "play none none reverse",
          markers: false, // Set to true for debugging
        },
      });

      // Stagger animations better
      tl.from(".subfooter-title", {
        y: 50,
        opacity: 0,
        duration: 0.8,
        ease: "power3.out",
      })
        .from(
          ".subfooter-desc",
          {
            y: 40,
            opacity: 0,
            duration: 0.7,
            ease: "power3.out",
          },
          "-=0.4",
        ) // Reduced overlap
        .from(
          ".subfooter-benefits-title",
          {
            y: 40,
            opacity: 0,
            duration: 0.6,
            ease: "power3.out",
          },
          "-=0.3",
        )
        .from(
          ".subfooter-benefit",
          {
            y: 30,
            opacity: 0,
            duration: 0.5,
            stagger: 0.2, // Increased stagger
            ease: "power3.out",
          },
          "-=0.2",
        )
        .from(
          ".subfooter-button",
          {
            y: 30,
            opacity: 0,
            duration: 0.6,
            ease: "power3.out",
          },
          "-=0.1",
        );
    }, sectionRef);

    return () => {
      ctx.revert();
      // Clean up ScrollTrigger instances
      ScrollTrigger.getAll().forEach((trigger) => trigger.kill());
    };
  }, []);

  // CSS classes for different vintage effects
  const vintageEffects = {
    sepia: "sepia-vintage-effect",
    vintage: "premium-vintage-effect",
    warm: "warm-vintage-effect",
    classic: "classic-vintage-effect",
    aged: "aged-paper-effect",
  };

  return (
    <section ref={sectionRef} className="relative overflow-hidden">
      <style jsx>{`
        /* Base vintage overlay */
        .vintage-overlay {
          position: absolute;
          inset: 0;
          background: linear-gradient(
            45deg,
            rgba(179, 137, 52, 0.08) 0%,
            rgba(232, 204, 123, 0.05) 25%,
            rgba(0, 0, 0, 0.05) 50%,
            rgba(139, 69, 19, 0.03) 75%
          );
          mix-blend-mode: overlay;
          z-index: 1;
        }

        /* Subtle grain texture for premium vintage feel */
        .grain-texture::before {
          content: "";
          position: absolute;
          inset: 0;
          background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.85' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)' opacity='0.08'/%3E%3C/svg%3E");
          opacity: 0.15; /* Reduced opacity */
          z-index: 2;
          pointer-events: none;
        }

        /* Effect 1: Sepia Vintage */
        .sepia-vintage-effect .bg-image {
          filter: sepia(0.4) brightness(0.9) contrast(1.1) saturate(0.8);
          transition: filter 0.7s ease;
        }

        /* Effect 2: Premium Vintage (Recommended) */
        .premium-vintage-effect .bg-image {
          filter: sepia(0.3) brightness(0.85) contrast(1.15) saturate(0.7)
            hue-rotate(-5deg);
          transition: filter 0.7s ease;
        }

        /* Effect 3: Warm Vintage */
        .warm-vintage-effect .bg-image {
          filter: sepia(0.25) brightness(0.9) contrast(1.05) saturate(0.9)
            hue-rotate(350deg);
          transition: filter 0.7s ease;
        }

        /* Effect 4: Classic Vintage */
        .classic-vintage-effect .bg-image {
          filter: sepia(0.5) brightness(0.8) contrast(1.2) saturate(0.6);
          transition: filter 0.7s ease;
        }

        /* Effect 5: Aged Paper Effect */
        .aged-paper-effect .bg-image {
          filter: sepia(0.35) brightness(0.88) contrast(1.1) saturate(0.75);
          transition: filter 0.7s ease;
        }

        /* Light vignette for depth */
        .vignette-overlay::after {
          content: "";
          position: absolute;
          inset: 0;
          background: radial-gradient(
            ellipse at center,
            transparent 40%,
            rgba(0, 0, 0, 0.15) 100% /* Reduced opacity */
          );
          pointer-events: none;
          z-index: 3;
        }

        /* Ensure content is above all backgrounds */
        .content-container {
          position: relative;
          z-index: 10;
        }

        /* Background container */
        .background-container {
          position: absolute;
          inset: 0;
          z-index: -1;
        }
      `}</style>

      {/* Background container with vintage effect */}
      <div
        className={`background-container ${vintageEffects[vintageEffect as keyof typeof vintageEffects]} grain-texture vignette-overlay`}
      >
        {/* Load only appropriate image */}
        {!isMobile ? (
          <img
            src="/images/subFooter.jpg"
            className="bg-image absolute inset-0 w-full h-full object-cover"
            alt="Vintage background"
            loading="lazy"
          />
        ) : (
          <img
            src="/images/subFooter2.png"
            className="bg-image absolute inset-0 w-full h-full object-cover"
            alt="Vintage background mobile"
            loading="lazy"
          />
        )}

        {/* Vintage color overlay */}
        <div className="vintage-overlay"></div>

        {/* Optional: Subtle gradient overlay that complements meat tones */}
        <div className="absolute inset-0 bg-linear-to-br from-[#B38934]/10 via-transparent to-[#2c1810]/10 mix-blend-mode-multiply"></div>
      </div>

      {/* Content */}
      <div className="content-container container mx-auto px-4 py-16 md:py-24">
        <div className="max-w-xl space-y-8">
          {/* Text content */}
          <div className="space-y-4">
            <h1 className="font-satoshi  subfooter-title text-3xl md:text-5xl font-semibold leading-tight text-white drop-shadow-lg">
              Elevate Protein With{" "}
              <p className="font-clearface font-light italic p-1 bg-linear-to-l from-[#B38934] to-[#E8CC7B] bg-clip-text text-transparent">
                Other Local Fresh Meat.
              </p>
            </h1>
          </div>

          <div className="space-y-6">
            <p className="subfooter-benefits-title text-lg md:text-2xl font-bold text-white drop-shadow">
              BENEFITS OF A SUBSCRIPTION
            </p>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                {
                  icon: "/icons/subFooter1.svg",
                  text: "Freshly cut after order",
                },
                {
                  icon: "/icons/subFooter2.svg",
                  text: "Wide variety quality meats",
                },
                {
                  icon: "/icons/subFooter2.svg",
                  text: "Real-time freshness tracking",
                },
              ].map((benefit, index) => (
                <div
                  key={index}
                  className="subfooter-benefit flex items-center gap-3"
                >
                  <img
                    src={benefit.icon}
                    alt=""
                    className="w-8 h-8 drop-shadow"
                  />
                  <p className="text-gray-200 text-sm drop-shadow">
                    {benefit.text}
                  </p>
                </div>
              ))}
            </div>

            <div className="pt-4 subfooter-button">
              <PrimaryButton
                primaryColor="#d4a64a"
                text="ORDER ONLINE"
                className="uppercase tracking-wider drop-shadow-lg hover:drop-shadow-xl transition-all"
                gradientToWhite={true}
                href="https://dhesimeats.restaurantsolutions.ca/"
              />
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default SubFooter;
